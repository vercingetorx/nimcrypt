...
#     multiple times on the same camelliaCbcCtx, blocks chain across calls.
#     To process independent messages, create a fresh ctx with newCamelliaCbcCtx.
...
#     with newCamelliaCtrCtx.
import ./camellia_core
import ./galois
#[
...
func keyLenToBits(len: int): int =
  case len
  of 16: 128
  of 24: 192
  of 32: 256
  else: raise newException(ValueError, "Camellia requires 16, 24, or 32 byte keys")

proc stateInit(state: var BlockState, key: openArray[byte], keylen: int): int =
  let bits = keyLenToBits(keylen)
  state.rounds = camelliaEKeygen(bits, key, state.ek)
  state.dk = state.ek
  state.rounds

proc camelliaEncryptBlock(rk: openArray[uint32], rounds: int, pt: openArray[byte], ct: var openArray[byte]) =
  camelliaEncryptBlock(rounds, pt, rk, ct)

proc camelliaDecryptBlock(rk: openArray[uint32], rounds: int, ct: openArray[byte], pt: var openArray[byte]) =
  camelliaDecryptBlock(rounds, ct, rk, pt)


type
  camelliaEcbCtx* = object # Electronic CodeBook
    key:            seq[byte]
    state:          BlockState
  camelliaCbcCtx* = object # Ciphertext Block Chaining
    key:            seq[byte]
    iv:             seq[byte]
    state:          BlockState
    previousBlock:  array[blocksize, byte]
    isEncryptState: bool
  camelliaCtrCtx* = object # Counter
    key:            seq[byte]
    nonce:          seq[byte]
    state:          BlockState
    ...
    isEncryptState: bool
  camelliaGcmCtx* = object # Galois/Counter Mode (AEAD)
    key:   seq[byte]
    iv:    seq[byte]
    state: BlockState
    H:     Block128           # E_K(0^128)
    J0:    Block128           # pre-counter block
  camelliaGcmSivCtx* = object # Camellia-GCM-SIV (AEAD, per-nonce key derivation)
    key:   seq[byte]          # key-generating key (16 or 32 bytes)
    nonce: seq[byte]          # 12 bytes
  camelliaXtsCtx* = object # XTS mode (IEEE P1619)
    key1:  seq[byte]          # 16 or 32 bytes
    key2:  seq[byte]          # 16 or 32 bytes
    st1:   BlockState         # data encryption key
    st2:   BlockState         # tweak encryption key
...
proc initPreviousBlock(ctx: var camelliaCbcCtx) =
  ## initialize previous block with IV
  for i, b in ctx.iv:
    ctx.previousBlock[i] = b


proc initCounter*(ctx: var camelliaCtrCtx) =
...
proc incrementCounter(ctx: var camelliaCtrCtx) =
...
proc encrypt*(ctx: camelliaEcbCtx, input: openArray[byte], output: var openArray[byte]) =
  ## ECB Mode
  ## encrypt in place
  ...
  for i in countup(0, input.len - 1, step=blocksize):
    camelliaEncryptBlock(ctx.state.ek, ctx.state.rounds, input[i ..< i + blocksize], blk)
    for j, b in blk:
      output[i + j] = b


proc encrypt*(ctx: camelliaEcbCtx, input: openArray[byte]): seq[byte] =
  ## ECB Mode
...
proc encrypt*(ctx: camelliaEcbCtx, input: string, output: var openArray[byte]) =
  ## ECB Mode
  ## encrypt in place
  encrypt(ctx, input.encodeBytes(), output)


proc encrypt*(ctx: camelliaEcbCtx, input: string): seq[byte] =
  ## ECB Mode
  ## returns ciphertext as new sequence
  return encrypt(ctx, input.toOpenArrayByte(0, input.len.pred))


proc decrypt*(ctx: camelliaEcbCtx, input: openArray[byte], output: var openArray[byte]) =
  ## ECB Mode
  ## decrypt in place
  if input.len mod blocksize != 0:
    raise newException(ValueError, "input length must be a multiple of 16")
  ...
  for i in countup(0, input.len.pred, step=blocksize):
    camelliaDecryptBlock(ctx.state.dk, ctx.state.rounds, input[i ..< i + blocksize], blk)
    for j, b in blk:
      output[i + j] = b


proc decrypt*(ctx: camelliaEcbCtx, input: openArray[byte]): seq[byte] =
  ## ECB Mode
  ## returns ciphertext as new sequence
  if input.len mod blocksize != 0:
    raise newException(ValueError, "input length must be a multiple of 16")
...
proc decrypt*(ctx: camelliaEcbCtx, input: string, output: var openArray[byte]) =
  ## ECB Mode
  ## decrypt in place
  decrypt(ctx, input.encodeBytes(), output)


proc decrypt*(ctx: camelliaEcbCtx, input: string): seq[byte] =
  ## ECB Mode
  ## returns ciphertext as new sequence
...
proc encrypt*(ctx: var camelliaCbcCtx, input: openArray[byte], output: var openArray[byte]) =
  ## CBC Mode
  ## encrypt in place
  if input.len mod blocksize != 0:
    raise newException(ValueError, "input length must be a multiple of 16")
  ...
  for i in countup(0, input.high, step=blocksize):
    ...
    camelliaEncryptBlock(ctx.state.ek, ctx.state.rounds, xorBlocks(input[i ..< i + blocksize], ctx.previousBlock), blk)
    for j, b in blk:
      output[i + j] = b
    ctx.previousBlock = blk


proc encrypt*(ctx: var camelliaCbcCtx, input: openArray[byte]): seq[byte] =
  ## CBC Mode
  ## returns ciphertext as new sequence
  if input.len mod blocksize != 0:
    raise newException(ValueError, "input length must be a multiple of 16")
...
proc encrypt*(ctx: var camelliaCbcCtx, input: string, output: var openArray[byte]) =
  ## CBC Mode
  ## encrypt in place
  encrypt(ctx, input.encodeBytes(), output)


proc encrypt*(ctx: var camelliaCbcCtx, input: string): seq[byte] =
  ## CBC Mode
  ## returns ciphertext as new sequence
  return encrypt(ctx, input.encodeBytes())


proc decrypt*(ctx: var camelliaCbcCtx, input: openArray[byte], output: var openArray[byte]) =
  ## CBC Mode
  ## decrypt in place
  if input.len mod blocksize != 0:
    raise newException(ValueError, "input length must be a multiple of 16")
  ...
  for i in countup(0, input.high, step=blocksize):
    for i, b in input[i ..< i + blocksize]:
      ptBlk[i] = b
    camelliaDecryptBlock(ctx.state.dk, ctx.state.rounds, ptBlk, ctBlk)
...
proc decrypt*(ctx: var camelliaCbcCtx, input: openArray[byte]): seq[byte] =
  ## CBC Mode
  ## returns ciphertext as new sequence
  if input.len mod blocksize != 0:
    raise newException(ValueError, "input length must be a multiple of 16")
...
proc decrypt*(ctx: var camelliaCbcCtx, input: string, output: var openArray[byte]) =
  ## CBC Mode
  ## decrypt in place
  decrypt(ctx, input.encodeBytes(), output)


proc decrypt*(ctx: var camelliaCbcCtx, input: string): seq[byte] =
  ## CBC Mode
  ## returns ciphertext as new sequence
  return decrypt(ctx, input.encodeBytes())
...
proc crypt*(ctx: var camelliaCtrCtx, input: openArray[byte], output: var openArray[byte]) =
  ## CTR Mode
  ## crypt in place
  if input.len > output.len:
    raise newException(ValueError, "output length must be >= input length")
  ...
  for i in countup(0, input.high, step=blocksize):
    # Encrypt the counter
    camelliaEncryptBlock(ctx.state.ek, ctx.state.rounds, ctx.counter, blk)
...
proc crypt*(ctx: var camelliaCtrCtx, input: openArray[byte]): seq[byte] =
  ## CTR Mode
...
proc encrypt*(ctx: var camelliaCtrCtx, input: openArray[byte], output: var openArray[byte]) =
  ## CTR Mode
  ## encrypt in place
  if not ctx.isEncryptState:
    ctx.initCounter()
...
proc encrypt*(ctx: var camelliaCtrCtx, input: openArray[byte]): seq[byte] =
  ## CTR Mode
  ## returns ciphertext as new sequence
  if not ctx.isEncryptState:
    ctx.initCounter()
...
proc encrypt*(ctx: var camelliaCtrCtx, input: string, output: var openArray[byte]) =
  ## CTR Mode
  ## encrypt in place
  if not ctx.isEncryptState:
    ctx.initCounter()
...
proc encrypt*(ctx: var camelliaCtrCtx, input: string): seq[byte] =
  ## CTR Mode
  ## returns ciphertext as new sequence
  if not ctx.isEncryptState:
    ctx.initCounter()
...
proc decrypt*(ctx: var camelliaCtrCtx, input: openArray[byte], output: var openArray[byte]) =
  ## CTR Mode
  ## decrypt in place
  if ctx.isEncryptState:
    ctx.initCounter()
...
proc decrypt*(ctx: var camelliaCtrCtx, input: openArray[byte]): seq[byte] =
  ## CTR Mode
  ## returns ciphertext as new sequence
  if ctx.isEncryptState:
    ctx.initCounter()
...
proc decrypt*(ctx: var camelliaCtrCtx, input: string, output: var openArray[byte]) =
  ## CTR Mode
  ## decrypt in place
  if ctx.isEncryptState:
    ctx.initCounter()
...
proc decrypt*(ctx: var camelliaCtrCtx, input: string): seq[byte] =
  ## CTR Mode
  ## returns ciphertext as new sequence
  if ctx.isEncryptState:
    ctx.initCounter()
...
proc gcmCalcH(state: BlockState): Block128 =
  var zero: Block128
  var outBlk: Block128
  camelliaEncryptBlock(state.ek, state.rounds, zero, outBlk)
...
proc gctr(state: BlockState, icb: Block128, input: openArray[byte], output: var openArray[byte]) =
  var counter: Block128 = icb
  var stream: Block128
  var i = 0
  while i < input.len:
    inc32(counter)
    camelliaEncryptBlock(state.ek, state.rounds, counter, stream)
...
proc encrypt*(ctx: var camelliaGcmCtx, aad: openArray[byte], plaintext: openArray[byte], ciphertext: var openArray[byte], tag: var openArray[byte]) =
  ...
  # T = E(K, J0) xor S
  var ekJ0: Block128
  camelliaEncryptBlock(ctx.state.ek, ctx.state.rounds, ctx.J0, ekJ0)
  for i in 0 ..< blocksize:
    tag[i] = ekJ0[i] xor S[i]


proc encrypt*(ctx: var camelliaGcmCtx, aad: openArray[byte], plaintext: openArray[byte]): (seq[byte], array[blocksize, byte]) =
...
proc decrypt*(ctx: var camelliaGcmCtx, aad: openArray[byte], ciphertext: openArray[byte], plaintext: var openArray[byte], tag: openArray[byte]) =
...
proc decrypt*(ctx: var camelliaGcmCtx, aad: openArray[byte], ciphertext: openArray[byte], tag: openArray[byte]): seq[byte] =
...
proc deriveGcmSivKeys*(kgen: openArray[byte], nonce: openArray[byte]): (seq[byte], Block128) =
  ...
  var st: BlockState
  discard stateInit(st, kgen, kgen.len)
  var blk: array[blocksize, byte]
  ...
  camelliaEncryptBlock(st.ek, st.rounds, blk, blkOut)
...
proc gcmSivCtr(encState: BlockState, initial: array[blocksize, byte], input: openArray[byte], output: var openArray[byte]) =
  var counter = initial
  var stream: array[blocksize, byte]
  var i = 0
  while i < input.len:
    # keystream = Camellia_K(counter)
    camelliaEncryptBlock(encState.ek, encState.rounds, counter, stream)
    let take = min(blocksize, input.len - i)
    for j in 0 ..< take:
      output[i + j] = input[i + j] xor stream[j]
    inc32Le(counter)
...
proc encrypt*(ctx: camelliaGcmSivCtx, aad: openArray[byte], plaintext: openArray[byte], ciphertext: var openArray[byte], tag: var openArray[byte]) =
  if plaintext.len > ciphertext.len:
    raise newException(ValueError, "output length must be >= input length")
  if tag.len != blocksize:
    raise newException(ValueError, "tag must be 16 bytes long")
  ...
  var encState: BlockState
  discard stateInit(encState, encKey, encKey.len)
  var tagOut: array[blocksize, byte]
  camelliaEncryptBlock(encState.ek, encState.rounds, tagBlock, tagOut)
...
proc encrypt*(ctx: camelliaGcmSivCtx, aad: openArray[byte], plaintext: openArray[byte]): (seq[byte], array[blocksize, byte]) =
  var ct = newSeq[byte](plaintext.len)
  var tag: array[blocksize, byte]
  var tagBuf = newSeq[byte](blocksize)
...
proc decrypt*(ctx: camelliaGcmSivCtx, aad: openArray[byte], ciphertext: openArray[byte], plaintext: var openArray[byte], tag: openArray[byte]) =
  if ciphertext.len > plaintext.len:
    raise newException(ValueError, "output length must be >= input length")
  if tag.len != blocksize:
    raise newException(ValueError, "tag must be 16 bytes long")
  ...
  camelliaEncryptBlock(encState.ek, encState.rounds, tagBlock, expected)
...
proc decrypt*(ctx: camelliaGcmSivCtx, aad: openArray[byte], ciphertext: openArray[byte], tag: openArray[byte]): seq[byte] =
  var pt = newSeq[byte](ciphertext.len)
...
proc encryptAead*(ctx: var camelliaGcmCtx, aad: openArray[byte], plaintext: openArray[byte]): seq[byte] =
  var ct = newSeq[byte](plaintext.len)
  var tagBuf = newSeq[byte](blocksize)
  ctx.encrypt(aad, plaintext, ct, tagBuf)
...
proc decryptAead*(ctx: var camelliaGcmCtx, aad: openArray[byte], data: openArray[byte]): seq[byte] =
...
proc encryptAeadHex*(ctx: var camelliaGcmCtx, aad, plaintext: string): string =
  let outBuf = ctx.encryptAead(aad.encodeBytes(), plaintext.encodeBytes())
  return hexDigest(outBuf)


proc decryptAeadHex*(ctx: var camelliaGcmCtx, aad: string, dataHex: string): string =
  let data = fromHex(dataHex)
  let pt = ctx.decryptAead(aad.encodeBytes(), data)
  return decodeBytes(pt)


proc encryptAeadB64*(ctx: var camelliaGcmCtx, aad, plaintext: string): string =
  let outBuf = ctx.encryptAead(aad.encodeBytes(), plaintext.encodeBytes())
  return encode(outBuf)


proc decryptAeadB64*(ctx: var camelliaGcmCtx, aad: string, dataB64: string): string =
...
proc encryptAead*(ctx: camelliaGcmSivCtx, aad: openArray[byte], plaintext: openArray[byte]): seq[byte] =
  var ct = newSeq[byte](plaintext.len)
  var tagBuf = newSeq[byte](blocksize)
  ctx.encrypt(aad, plaintext, ct, tagBuf)
  result = newSeq[byte](ct.len + blocksize)
...
proc decryptAead*(ctx: camelliaGcmSivCtx, aad: openArray[byte], data: openArray[byte]): seq[byte] =
  if data.len < blocksize:
    raise newException(ValueError, "aead input too short (no tag)")
  let ctLen = data.len - blocksize
  var pt = newSeq[byte](ctLen)
...
proc encryptAeadHex*(ctx: camelliaGcmSivCtx, aad, plaintext: string): string =
  let outBuf = ctx.encryptAead(aad.encodeBytes(), plaintext.encodeBytes())
  return hexDigest(outBuf)


proc decryptAeadHex*(ctx: camelliaGcmSivCtx, aad: string, dataHex: string): string =
  let data = fromHex(dataHex)
  let pt = ctx.decryptAead(aad.encodeBytes(), data)
  return decodeBytes(pt)
...
proc encrypt*(ctx: camelliaXtsCtx, tweak: openArray[byte], input: openArray[byte], output: var openArray[byte]) =
  ...
  camelliaEncryptBlock(ctx.st2.ek, ctx.st2.rounds, t, t)
  ...
  while off < fullLen:
    ...
    camelliaEncryptBlock(ctx.st1.ek, ctx.st1.rounds, scratch, scratch)
...
proc decrypt*(ctx: camelliaXtsCtx, tweak: openArray[byte], input: openArray[byte], output: var openArray[byte]) =
  ...
  while off < fullLen:
    for i in 0 ..< blocksize: scratch[i] = input[off + i] xor t[i]
    camelliaDecryptBlock(ctx.st1.dk, ctx.st1.rounds, scratch, scratch)
    for i in 0 ..< blocksize: output[off + i] = scratch[i] xor t[i]
    xtsMulAlpha(t)
    off += blocksize
...
proc encrypt*(ctx: camelliaXtsCtx, tweak: openArray[byte], input: openArray[byte]): seq[byte] =
  result = newSeq[byte](input.len)
  encrypt(ctx, tweak, input, result)


proc decrypt*(ctx: camelliaXtsCtx, tweak: openArray[byte], input: openArray[byte]): seq[byte] =
  result = newSeq[byte](input.len)
  decrypt(ctx, tweak, input, result)


proc encrypt*(ctx: camelliaXtsCtx, tweak, input: string): seq[byte] =
  return encrypt(ctx, tweak.encodeBytes(), input.encodeBytes())


proc decrypt*(ctx: camelliaXtsCtx, tweak, input: string): seq[byte] =
  return decrypt(ctx, tweak.encodeBytes(), input.encodeBytes())


# XTS: string helpers (hex/base64)
proc encryptXtsHex*(ctx: camelliaXtsCtx, tweakHex, plaintextHex: string): string =
...
proc decryptXtsHex*(ctx: camelliaXtsCtx, tweakHex, dataHex: string): string =
  let tweak = fromHex(tweakHex)
...
proc encryptXtsB64*(ctx: camelliaXtsCtx, tweakB64, plaintextB64: string): string =
...
proc decryptXtsB64*(ctx: camelliaXtsCtx, tweakB64, dataB64: string): string =
  let tweakRaw = decode(tweakB64)
...
proc encryptAeadB64*(ctx: camelliaGcmSivCtx, aad, plaintext: string): string =
  let outBuf = ctx.encryptAead(aad.encodeBytes(), plaintext.encodeBytes())
  return encode(outBuf)


proc decryptAeadB64*(ctx: camelliaGcmSivCtx, aad: string, dataB64: string): string =
  let raw = decode(dataB64)
  let data = encodeBytes(raw)
  let pt = ctx.decryptAead(aad.encodeBytes(), data)
  return decodeBytes(pt)
...
proc newCamelliaEcbCtx*(key: openArray[byte]): camelliaEcbCtx =
...
proc newCamelliaEcbCtx*(key: string): camelliaEcbCtx =
  return newCamelliaEcbCtx(key.encodeBytes())


proc newCamelliaCbcCtx*(key, iv: openArray[byte]): camelliaCbcCtx =
  if not key.len in {16, 24, 32}:
    raise newException(ValueError, "Key must be 16/24/32 bytes long")
  if iv.len != 16:
...
proc newCamelliaCbcCtx*(key, iv: string): camelliaCbcCtx =
  return newCamelliaCbcCtx(key.encodeBytes(), iv.encodeBytes())


proc newCamelliaCtrCtx*(key, nonce: openArray[byte], initState: openArray[byte]=newSeq[byte](8)): camelliaCtrCtx =
  if not key.len in {16, 24, 32}:
    raise newException(ValueError, "Key must be 16/24/32 bytes long")
  if nonce.len != 8:
...
proc newCamelliaCtrCtx*(key, nonce: string, initState: int = 0): camelliaCtrCtx =
  return newCamelliaCtrCtx(key.encodeBytes(), nonce.encodeBytes(), intToBytesBE(uint64(initState)))


proc newCamelliaGcmCtx*(key, iv: openArray[byte]): camelliaGcmCtx =
  if not key.len in {16, 24, 32}:
    raise newException(ValueError, "Key must be 16/24/32 bytes long")
  if iv.len == 0:
...
proc newCamelliaGcmCtx*(key, iv: string): camelliaGcmCtx =
  return newCamelliaGcmCtx(key.encodeBytes(), iv.encodeBytes())


proc newCamelliaGcmSivCtx*(key, nonce: openArray[byte]): camelliaGcmSivCtx =
...
proc newCamelliaGcmSivCtx*(key, nonce: string): camelliaGcmSivCtx =
  return newCamelliaGcmSivCtx(key.encodeBytes(), nonce.encodeBytes())


proc newCamelliaXtsCtx*(key1, key2: openArray[byte]): camelliaXtsCtx =
  ...
  discard stateInit(result.st1, result.key1, result.key1.len)
  discard stateInit(result.st2, result.key2, result.key2.len)


proc newCamelliaXtsCtx*(combined: openArray[byte]): camelliaXtsCtx =
  ...
  return newCamelliaXtsCtx(combined[0 ..< half], combined[half ..< combined.len])


proc newCamelliaXtsCtx*(key1, key2: string): camelliaXtsCtx =
  return newCamelliaXtsCtx(key1.encodeBytes(), key2.encodeBytes())


proc newCamelliaXtsCtx*(combined: string): camelliaXtsCtx =
  return newCamelliaXtsCtx(combined.encodeBytes())
...
when isMainModule:
  ...
  var ctx = newCamelliaCbcCtx(key, iv)
  
  let ciphertext = ctx.encrypt(message)
  echo encode(ciphertext)
  doAssert encode(ciphertext) == "FBqyJSWw4aMaaxpvOt2ZCKgYAHWkuPP6LnIL13ZStCk="
