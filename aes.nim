...
#     multiple times on the same aesCbcCtx, blocks chain across calls.
#     To process independent messages, create a fresh ctx with newAesCbcCtx.
...
type
  ...
  aesCtrCtx* = object # Counter
    ...
    isEncryptState: bool
  aesGcmCtx* = object # Galois/Counter Mode (AEAD)
    key:   seq[byte]
    iv:    seq[byte]
    state: BlockState
    H:     Block128           # E_K(0^128)
    J0:    Block128           # pre-counter block
  aesGcmSivCtx* = object # AES-GCM-SIV (AEAD, per-nonce key derivation)
    key:   seq[byte]          # key-generating key (16 or 32 bytes)
    nonce: seq[byte]          # 12 bytes
  aesXtsCtx* = object # XTS mode (IEEE P1619)
...
proc gcmCalcH(state: BlockState): Block128 =
  var zero: Block128
  var outBlk: Block128
  rijndaelEncrypt(state.ek, state.rounds, zero, outBlk)
...
proc gctr(state: BlockState, icb: Block128, input: openArray[byte], output: var openArray[byte]) =
  ...
  while i < input.len:
    inc32(counter)
    rijndaelEncrypt(state.ek, state.rounds, counter, stream)
...
proc encrypt*(ctx: var aesGcmCtx, aad: openArray[byte], plaintext: openArray[byte], ciphertext: var openArray[byte], tag: var openArray[byte]) =
  ...
  # T = E(K, J0) xor S
  var ekJ0: Block128
  rijndaelEncrypt(ctx.state.ek, ctx.state.rounds, ctx.J0, ekJ0)
  for i in 0 ..< blocksize:
    tag[i] = ekJ0[i] xor S[i]


proc encrypt*(ctx: var aesGcmCtx, aad: openArray[byte], plaintext: openArray[byte]): (seq[byte], array[blocksize, byte]) =
...
proc decrypt*(ctx: var aesGcmCtx, aad: openArray[byte], ciphertext: openArray[byte], plaintext: var openArray[byte], tag: openArray[byte]) =
...
proc decrypt*(ctx: var aesGcmCtx, aad: openArray[byte], ciphertext: openArray[byte], tag: openArray[byte]): seq[byte] =
...
proc deriveGcmSivKeys*(kgen: openArray[byte], nonce: openArray[byte]): (seq[byte], Block128) =
  ...
  rijndaelEncrypt(st.ek, st.rounds, blk, blkOut)
...
proc gcmSivCtr(encState: BlockState, initial: array[blocksize, byte], input: openArray[byte], output: var openArray[byte]) =
  ...
  while i < input.len:
    ...
    rijndaelEncrypt(encState.ek, encState.rounds, counter, stream)
    let take = min(blocksize, input.len - i)
    for j in 0 ..< take:
      output[i + j] = input[i + j] xor stream[j]
    inc32Le(counter)
...
proc encrypt*(ctx: aesGcmSivCtx, aad: openArray[byte], plaintext: openArray[byte], ciphertext: var openArray[byte], tag: var openArray[byte]) =
  if plaintext.len > ciphertext.len:
    raise newException(ValueError, "output length must be >= input length")
  if tag.len != blocksize:
    raise newException(ValueError, "tag must be 16 bytes long")
  ...
  rijndaelEncrypt(encState.ek, encState.rounds, tagBlock, tagOut)
...
proc encrypt*(ctx: aesGcmSivCtx, aad: openArray[byte], plaintext: openArray[byte]): (seq[byte], array[blocksize, byte]) =
  var ct = newSeq[byte](plaintext.len)
  var tag: array[blocksize, byte]
  var tagBuf = newSeq[byte](blocksize)
...
proc decrypt*(ctx: aesGcmSivCtx, aad: openArray[byte], ciphertext: openArray[byte], plaintext: var openArray[byte], tag: openArray[byte]) =
  if ciphertext.len > plaintext.len:
    raise newException(ValueError, "output length must be >= input length")
  if tag.len != blocksize:
    raise newException(ValueError, "tag must be 16 bytes long")
  ...
  rijndaelEncrypt(encState.ek, encState.rounds, tagBlock, expected)
...
proc decrypt*(ctx: aesGcmSivCtx, aad: openArray[byte], ciphertext: openArray[byte], tag: openArray[byte]): seq[byte] =
  var pt = newSeq[byte](ciphertext.len)
...
proc encryptAead*(ctx: var aesGcmCtx, aad: openArray[byte], plaintext: openArray[byte]): seq[byte] =
  var ct = newSeq[byte](plaintext.len)
  var tagBuf = newSeq[byte](blocksize)
  ctx.encrypt(aad, plaintext, ct, tagBuf)
...
proc decryptAead*(ctx: var aesGcmCtx, aad: openArray[byte], data: openArray[byte]): seq[byte] =
...
proc encryptAeadHex*(ctx: var aesGcmCtx, aad, plaintext: string): string =
  let outBuf = ctx.encryptAead(aad.encodeBytes(), plaintext.encodeBytes())
  return hexDigest(outBuf)


proc decryptAeadHex*(ctx: var aesGcmCtx, aad: string, dataHex: string): string =
  let data = fromHex(dataHex)
  let pt = ctx.decryptAead(aad.encodeBytes(), data)
  return decodeBytes(pt)


proc encryptAeadB64*(ctx: var aesGcmCtx, aad, plaintext: string): string =
  let outBuf = ctx.encryptAead(aad.encodeBytes(), plaintext.encodeBytes())
  return encode(outBuf)


proc decryptAeadB64*(ctx: var aesGcmCtx, aad: string, dataB64: string): string =
...
proc encryptAead*(ctx: aesGcmSivCtx, aad: openArray[byte], plaintext: openArray[byte]): seq[byte] =
  var ct = newSeq[byte](plaintext.len)
  var tagBuf = newSeq[byte](blocksize)
  ctx.encrypt(aad, plaintext, ct, tagBuf)
  result = newSeq[byte](ct.len + blocksize)
...
proc decryptAead*(ctx: aesGcmSivCtx, aad: openArray[byte], data: openArray[byte]): seq[byte] =
  if data.len < blocksize:
    raise newException(ValueError, "aead input too short (no tag)")
  let ctLen = data.len - blocksize
  var pt = newSeq[byte](ctLen)
...
proc encryptAeadHex*(ctx: aesGcmSivCtx, aad, plaintext: string): string =
  let outBuf = ctx.encryptAead(aad.encodeBytes(), plaintext.encodeBytes())
  return hexDigest(outBuf)


proc decryptAeadHex*(ctx: aesGcmSivCtx, aad: string, dataHex: string): string =
  let data = fromHex(dataHex)
  let pt = ctx.decryptAead(aad.encodeBytes(), data)
  return decodeBytes(pt)
...
proc encrypt*(ctx: aesXtsCtx, tweak: openArray[byte], input: openArray[byte], output: var openArray[byte]) =
  ...
  rijndaelEncrypt(ctx.st2.ek, ctx.st2.rounds, t, t)
  ...
  while off < fullLen:
    ...
    rijndaelEncrypt(ctx.st1.ek, ctx.st1.rounds, scratch, scratch)
...
proc decrypt*(ctx: aesXtsCtx, tweak: openArray[byte], input: openArray[byte], output: var openArray[byte]) =
  ...
  while off < fullLen:
    for i in 0 ..< blocksize: scratch[i] = input[off + i] xor t[i]
    rijndaelDecrypt(ctx.st1.dk, ctx.st1.rounds, scratch, scratch)
    for i in 0 ..< blocksize: output[off + i] = scratch[i] xor t[i]
    xtsMulAlpha(t)
    off += blocksize
...
proc encrypt*(ctx: aesXtsCtx, tweak: openArray[byte], input: openArray[byte]): seq[byte] =
  result = newSeq[byte](input.len)
  encrypt(ctx, tweak, input, result)


proc decrypt*(ctx: aesXtsCtx, tweak: openArray[byte], input: openArray[byte]): seq[byte] =
  result = newSeq[byte](input.len)
  decrypt(ctx, tweak, input, result)


proc encrypt*(ctx: aesXtsCtx, tweak, input: string): seq[byte] =
  return encrypt(ctx, tweak.encodeBytes(), input.encodeBytes())


proc decrypt*(ctx: aesXtsCtx, tweak, input: string): seq[byte] =
  return decrypt(ctx, tweak.encodeBytes(), input.encodeBytes())


# XTS: string helpers (hex/base64)
proc encryptXtsHex*(ctx: aesXtsCtx, tweakHex, plaintextHex: string): string =
...
proc decryptXtsHex*(ctx: aesXtsCtx, tweakHex, dataHex: string): string =
  let tweak = fromHex(tweakHex)
...
proc encryptXtsB64*(ctx: aesXtsCtx, tweakB64, plaintextB64: string): string =
...
proc decryptXtsB64*(ctx: aesXtsCtx, tweakB64, dataB64: string): string =
  let tweakRaw = decode(tweakB64)
...
proc encryptAeadB64*(ctx: aesGcmSivCtx, aad, plaintext: string): string =
  let outBuf = ctx.encryptAead(aad.encodeBytes(), plaintext.encodeBytes())
  return encode(outBuf)


proc decryptAeadB64*(ctx: aesGcmSivCtx, aad: string, dataB64: string): string =
  let raw = decode(dataB64)
  let data = encodeBytes(raw)
  let pt = ctx.decryptAead(aad.encodeBytes(), data)
  return decodeBytes(pt)
...
proc newAesGcmCtx*(key, iv: openArray[byte]): aesGcmCtx =
  if not key.len in {16, 24, 32}:
    raise newException(ValueError, "Key must be 16/24/32 bytes long")
  if iv.len == 0:
...
proc newAesGcmCtx*(key, iv: string): aesGcmCtx =
  return newAesGcmCtx(key.encodeBytes(), iv.encodeBytes())


proc newAesGcmSivCtx*(key, nonce: openArray[byte]): aesGcmSivCtx =
...
proc newAesGcmSivCtx*(key, nonce: string): aesGcmSivCtx =
  return newAesGcmSivCtx(key.encodeBytes(), nonce.encodeBytes())


proc newAesXtsCtx*(key1, key2: openArray[byte]): aesXtsCtx =
...
proc newAesXtsCtx*(combined: openArray[byte]): aesXtsCtx =
  ...
  return newAesXtsCtx(combined[0 ..< half], combined[half ..< combined.len])


proc newAesXtsCtx*(key1, key2: string): aesXtsCtx =
  return newAesXtsCtx(key1.encodeBytes(), key2.encodeBytes())


proc newAesXtsCtx*(combined: string): aesXtsCtx =
  return newAesXtsCtx(combined.encodeBytes())
